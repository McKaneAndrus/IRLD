...
def visualize_q_diff(data, out_file, labels):
    for b, bulk in enumerate(data):
        all_iters, all_models, true_qs = bulk
        ...
        plot = sns.lineplot(x='step_num', y='q_diffs', data=df, label=labels[b])
...
def main(out_dir, _run, experiment_nums, plot_all, labels):
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    data = [load_qs(experiment_set) for experiment_set in experiment_nums]
    out_file = os.path.join(out_dir, "{}_q_diff_visualization_from_{}.png".format( "_".join(str(x) for x in experiment_nums), _run._id))
    visualize_q_diff(data, out_file, labels)
