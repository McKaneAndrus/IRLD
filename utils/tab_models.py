...
class TabularInverseDynamicsLearner():
    ...
    def train(self, n_training_iters, sas_obs, adt_obs, batch_size, val_sas, val_adt, out_dir,
              transition_likelihood_weight=1.0, _run=None, true_qs=None, tab_save_freq=25, verbose=True):
        ...
        try:
            ...
            for train_time in range(n_training_iters):
                ...
                t_thetas += self.alpha * (transition_likelihood_weight * dT_trans + dT_pol)

                _run.log_scalar('val_likelihoods', val_likelihood, train_time)
                _run.log_scalar('val_nall', -vp_ll, train_time)
                _run.log_scalar('val_ntll', -vt_ll, train_time)
